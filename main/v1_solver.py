# -*- coding: utf-8 -*-
"""
v1_solver.py — 航母主控版（本地 / Web 通用）
============================================================
角色定位：
- V1 幾何系統的「實戰主控航母」
- 可用於：
  1️⃣ 本地 Terminal 測試（print 輸出）
  2️⃣ Web / API 呼叫（HTML 輸出）

設計重點：
- 整合四大幾何工廠：A / B / C / D
- 清楚定義工廠執行順序與資料依賴
- 同一套邏輯，同時支援「人看」與「前端顯示」
============================================================
"""

import importlib

# ==========================================================
# 📦 直接匯入 V1 四大加工廠（穩定版）
# ----------------------------------------------------------
# 說明：
# - 這支 solver 是「實戰穩定版」
# - 不走動態版本切換
# - 明確指定 v1 公式模組
# ==========================================================
from main.formulas.v1 import module_a_front_fillet
from main.formulas.v1 import module_b_outer_perp_R
from main.formulas.v1 import module_c_front_slope_endline_arc_v1
from main.formulas.v1 import module_d_angle_to_z_offset


# ==========================================================
# 🧠 主運算流程：依序執行 A → B → C → D
# ==========================================================
def run_all_modules(data):
    """
    主運算流程（CLI / Debug 專用）：

    輸入：
    - data：前端或本地提供的參數 dict

    特點：
    - 逐工廠 print 計算過程
    - 適合在 Terminal / 開發階段檢查數值
    """

    results = {}

    print("\n=== 🚀 開始執行所有幾何工廠 ===\n")

    # -------------------------------
    # 🏭 A 工廠
    # -------------------------------
    # 功能：
    # - 前端 R 角 + 斜度幾何建立
    # - 產生後續 C 工廠所需基準點
    print("【A】前端 R角斜度計算：")
    res_a = module_a_front_fillet.solve_a(data)

    # 將 A 工廠計算過程逐行印出
    for line in res_a["text_lines"]:
        print("   " + line)

    # 只收集 values 作為正式結果
    results["A"] = res_a["values"]
    print()


    # -------------------------------
    # 🏭 B 工廠
    # -------------------------------
    # 功能：
    # - 外徑垂直端
    # - 圓弧 / R 補正相關幾何
    print("【B】外徑垂直端 R 計算：")
    res_b = module_b_outer_perp_R.solve(data)

    for line in res_b["text_lines"]:
        print("   " + line)

    results["B"] = res_b["values"]
    print()


    # -------------------------------
    # 🏭 C 工廠（依賴 A 工廠結果）
    # -------------------------------
    # 功能：
    # - 斜角 → 平線 → 終點圓弧
    # - 需要 A 工廠產生的關鍵幾何（如 B7）
    print("【C】斜角 → 平線 → 終點弧 計算：")

    # 將 A 工廠結果寫回 data，供 C 使用
    data["A"] = results["A"]

    res_c = module_c_front_slope_endline_arc_v1.solve(data)

    for line in res_c["text_lines"]:
        print("   " + line)

    results["C"] = res_c["values"]
    print()


    # -------------------------------
    # 🏭 D 工廠
    # -------------------------------
    # 功能：
    # - 角度 → Z 軸距離
    # - 對應 Excel B14 類型的幾何補正
    print("【D】角度 Z軸距離 B14：")

    res_d = module_d_angle_to_z_offset.solve(data)

    for line in res_d["text_lines"]:
        print("   " + line)

    results["D"] = res_d["values"]
    print()

    print("=== ✅ 所有工廠完成 ===")
    return results


# ==========================================================
# 🌐 Web 專用輸出（結果 → HTML 字串）
# ==========================================================
def run_all_modules_with_output(data):
    """
    Web / API 專用入口：

    功能：
    - 重新載入所有工廠（開發熱更新）
    - 呼叫 run_all_modules 取得完整結果
    - 將結果整理為「前端可直接顯示的文字格式」
    """

    # ------------------------------------------------------
    # 🔄 熱更新四大工廠（避免重啟伺服器）
    # ------------------------------------------------------
    importlib.reload(module_a_front_fillet)
    importlib.reload(module_b_outer_perp_R)
    importlib.reload(module_c_front_slope_endline_arc_v1)
    importlib.reload(module_d_angle_to_z_offset)

    # 執行完整幾何運算
    results = run_all_modules(data)

    # 依工廠拆出結果
    A = results.get("A", {})
    B = results.get("B", {})
    C = results.get("C", {})
    D = results.get("D", {})

    # ------------------------------------------------------
    # 安全取值工具（避免 KeyError）
    # ------------------------------------------------------
    def g(dic, key, default=0):
        return dic.get(key, default)

    # ------------------------------------------------------
    # 組合前端顯示文字（等同 Excel 輸出表）
    # ------------------------------------------------------
    html = ""
    html += "=== 🧩 刀點座標總表 ===\n\n"

    # ===== 前端 R 角 =====
    html += f"前端R角起始點     G0 X{g(A,'B21'):.3f} Z2. \n"
    html += f"前端R角起始點     G1 Z{g(A,'B22'):.3f} F0.1\n\n"

    html += f"前端R角終點       G03 X{g(A,'B42'):.3f} Z{g(A,'B43'):.3f} R{g(A,'B44'):.3f} F0.1\n"
    html += f"前端R角終點        \n"
    html += f"前端R角刀鼻補正    \n\n"

    # ===== 未端 R 角 =====
    html += f"未端R角起始點      G1 X{g(C,'E3'):.3f} Z{g(C,'E2'):.3f} F0.1\n"
    html += f"未端R角起始點      \n\n"

    html += f"未端R角終點        G03 X{g(C,'E5'):.3f} Z{g(C,'E4'):.3f} R{g(C,'E6'):.3f} F0.1\n"
    html += f"未端R角終點        \n"
    html += f"刀鼻補正後圓孤     \n\n"

    # ===== 外徑圓弧補正 =====
   
    #=html += f"圓孤R前端          X={g(B,'OUT_E4'):.3f}\n"=
    html += f"圓孤R前端          G1 Z-{g(B,'OUT_E3'):.3f} F0.1\n\n"
    html += f"圓孤後x座標:       G02 X{g(B,'OUT_E7'):.3f} Z-{g(B,'OUT_E6'):.3f} R{g(B,'OUT_E9'):.3f} F0.1\n"
    html += f"圓孤後z座標:       \n"
    html += f"刀鼻補正後圓孤:    \n\n"

    html += "=== 🟢 幾何運算完成 ==="
    
    return html
