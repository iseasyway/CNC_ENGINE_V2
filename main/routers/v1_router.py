# main/routers/v1_router.py
from fastapi import APIRouter, Request, Form
from fastapi.responses import HTMLResponse, JSONResponse
from fastapi.templating import Jinja2Templates

from main.v1_master_solver import run_all_modules_with_output
...
def v1_page(request: Request, module: str | None = None):
    """
    module:
      a1 = 前端圓角
      a2 = 前端倒角
...
# V1 計算 API（不動）
# =========================
@router.post("/api/v1/calc")
async def api_v1_calc(
    前端x軸外徑: float = Form(...),
    斜度長: float = Form(...),
    角度: float = Form(...),
    前端R角: float = Form(...),
